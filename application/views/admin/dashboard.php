<div class="content-wrapper" style="background:#f5f7fa; overflow-x:hidden;">
        ...
        <a href="<?= site_url('admin/barang_masuk') ?>" class="notif-bar">
            ...
            <div class="notif-bar-tags">
                <?php foreach (array_slice($suppliers_unik, 0, 3) as $sup): ?>
                    <span class="notif-tag"><i class="fas fa-truck"
                            style="font-size:9px;"></i><?= htmlspecialchars($sup) ?></span>
                <?php endforeach; ?>
    ...
    <?php if (!empty($produk_rugi)): ?>
        <a href="<?= site_url('admin/produk') ?>" class="notif-bar"
            style="background:linear-gradient(135deg,#fef2f2,#fee2e2); border-color:#fca5a5;">
            <div class="notif-pulse" style="background:#dc2626;">
                <i class="fas fa-exclamation-triangle"></i>
            </div>
            <div class="notif-bar-text">
                <strong style="color:#991b1b;">
                    <i class="fas fa-chart-line-down" style="margin-right:4px;"></i>⚠️ <?= count($produk_rugi) ?> produk
                    harga jualnya di bawah/sama dengan harga modal!
                </strong>
            ...
            <div class="notif-bar-tags">
                <?php foreach (array_slice($produk_rugi, 0, 3) as $pr): ?>
                    <span class="notif-tag" style="background:rgba(220,38,38,0.1); border-color:#fca5a5; color:#b91c1c;">
                        ...
                        <small style="opacity:0.7;">(Jual: <?= number_format($pr['harga_jual'], 0, ',', '.') ?> / Modal:
                            <?= number_format($pr['harga_beli'], 0, ',', '.') ?>)</small>
                    </span>
                <?php endforeach; ?>
                <?php if (count($produk_rugi) > 3): ?>
                    <span class="notif-tag"
                        style="background:rgba(220,38,38,0.15); border-color:#fca5a5; color:#b91c1c;">+<?= count($produk_rugi) - 3 ?>
                        lainnya</span>
                <?php endif; ?>
            </div>
    ...
    <div class="dash-main-section">
        ...
        <div style="display:flex; flex-direction:column; gap:20px; min-width:0;">
            ...
            <div class="monitoring-card">
                <div class="monitoring-header">
                    ...
                    <?php endif; ?>
                    <a href="<?= site_url('admin/barang_masuk') ?>" class="btn-card-action"
                        style="margin-left:<?= $bm_count > 0 ? '8px' : 'auto' ?>;">Semua →</a>
                </div>

                <?php if (!empty($bm_items_today)): ?>
                    <div
                        style="padding:10px 18px; border-bottom:1px solid #f0fdf4; display:flex; align-items:center; gap:8px; flex-wrap:wrap;">
                        <button onclick="toggleBMView('items')" id="btnItems" class="filter-bm-btn active-bm-btn"
                            style="font-size:11.5px;font-weight:600;padding:5px 12px;border-radius:20px;border:1.5px solid #86efac;background:#16a34a;color:#fff;cursor:pointer;font-family:'DM Sans',sans-serif;transition:all 0.15s;white-space:nowrap;">
                            <i class="fas fa-cubes"></i> Per Produk
                        </button>
                        <button onclick="toggleBMView('trx')" id="btnTrx"
                            style="font-size:11.5px;font-weight:600;padding:5px 12px;border-radius:20px;border:1.5px solid #e5e7eb;background:#fff;color:#6b7280;cursor:pointer;font-family:'DM Sans',sans-serif;transition:all 0.15s;white-space:nowrap;">
                        ...
                        <?php foreach (array_slice($bm_items_today, 0, 8) as $item): ?>
                            <div class="bm-item">
                                ...
                                <div style="flex:1;min-width:0;">
                                    ...
                                    <span class="bm-produk-supplier"><i class="fas fa-truck"
                                            style="font-size:9px;margin-right:3px;"></i><?= htmlspecialchars($item['nama_supplier']) ?></span>
                        ...
                        <?php if (count($bm_items_today) > 8): ?>
                            <div style="padding:10px 18px;text-align:center;">
                                <a href="<?= site_url('admin/barang_masuk') ?>"
                                    ...
                                    +<?= count($bm_items_today) - 8 ?> produk lainnya — Lihat Semua <i
                                        class="fas fa-arrow-right" style="font-size:10px;"></i>
                                </a>
                            </div>
                        <?php endif; ?>
                    </div>
                    ...
                    <div id="viewTrx" style="display:none;">
                        <?php foreach ($bm_hari_ini as $trx): ?>
                            <a href="<?= site_url('admin/barang_masuk/detail/' . $trx['id_purchase']) ?>" class="bm-trx-row"
                                ...
                                <div style="margin-left:10px;flex:1;min-width:0;">
                                    ...
                                    <span class="bm-trx-supplier"
                                        style="display:block;font-size:11px;margin-top:2px;"><?= htmlspecialchars($trx['nama_supplier']) ?>
                                        &bull; by <?= htmlspecialchars($trx['nama_input']) ?></span>
                                </div>
                                <div style="text-align:right;flex-shrink:0;">
                                    <span class="bm-trx-total">Rp <?= number_format($trx['total_bayar'], 0, ',', '.') ?></span>
                                    <span class="bm-trx-time"
                                        style="display:block;margin-top:2px;"><?= date('H:i', strtotime($trx['created_at'])) ?></span>
                                </div>
                            </a>
                        <?php endforeach; ?>
                    </div>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="5" style="text-align:center;padding:32px;color:#9ca3af;font-size:13px;">
                                        Belum terdapat data transaksi penjualan.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
        ...
        <div class="dash-right-col">
            ...
            <div class="dash-card">
                ...
                <div class="quick-stat-item">
                    <div class="qs-icon" style="background:#f0fdf4; color:#16a34a;"><i
                            class="fas fa-money-bill-wave"></i></div>
                    <span class="qs-label">Pendapatan</span>
                    <span class="qs-value" style="font-size:12px;">Rp
                        <?= number_format($pendapatan_hari ?? 0, 0, ',', '.') ?></span>
                </div>
                <div class="quick-stat-item" style="position:relative;">
                    ...
                    <span class="qs-value" style="color:<?= $bm_count > 0 ? '#16a34a' : '#9ca3af' ?>;">
                        <?= $bm_count ?> faktur
                        <?php if ($bm_count > 0): ?>
                            <span style="display:block;font-size:10.5px;color:#16a34a;font-weight:500;"><?= $bm_qty_total ?>
                                unit</span>
                        <?php endif; ?>
                    </span>
                    <?php if ($bm_count > 0): ?>
                        <span
                            style="position:absolute;top:12px;right:18px;width:8px;height:8px;background:#16a34a;border-radius:50%;box-shadow:0 0 0 3px rgba(22,163,74,0.2);animation:pulseRing 2s ease infinite;"></span>
                    <?php endif; ?>
                </div>
                <div class="quick-stat-item">
            ...
            <div class="dash-card">
                ...
                <?php if (!empty($produk_habis_list)): ?>
                    <div
                        style="margin:0 16px 12px; padding:12px 16px; background:#fef2f2; border:1px solid #fecaca; border-radius:10px; display:flex; align-items:flex-start; gap:10px;">
                        <i class="fas fa-exclamation-circle"
                            style="color:#dc2626; font-size:16px; margin-top:2px; flex-shrink:0;"></i>
                        <div style="flex:1;min-width:0;">
                            <div style="font-size:12.5px; font-weight:700; color:#dc2626; margin-bottom:4px;">
                                <?= count($produk_habis_list) ?> Produk Stok Habis
                            </div>
                            <div
                                style="font-size:11.5px; color:#991b1b; line-height:1.5; display:flex; flex-wrap:wrap; gap:4px;">
                                <?php foreach (array_slice($produk_habis_list, 0, 3) as $ph): ?>
                                    <span
                                        style="background:#fee2e2; padding:2px 8px; border-radius:5px; font-weight:600;"><?= htmlspecialchars($ph['nama_produk']) ?></span>
                                <?php endforeach; ?>
                                <?php if (count($produk_habis_list) > 3): ?>
                                    <span style="font-weight:600; color:#b91c1c;">+<?= count($produk_habis_list) - 3 ?>
                                        lainnya</span>
                                <?php endif; ?>
                            </div>
                        </div>
                ...
                <?php if (!empty($produk_menipis)): ?>
                    <?php foreach (array_slice($produk_menipis, 0, 5) as $p): ?>
                        <div class="low-stock-item">
                            <div class="qs-icon" style="background:#fef2f2; color:#dc2626; flex-shrink:0;"><i
                            ...
                            <div class="low-stock-bar-wrap">
                                ...
                                <div class="progress-mini">
                                    <div class="progress-mini-fill <?= $p['stok'] <= 3 ? 'critical' : '' ?>"
                                        style="width:<?= min(($p['stok'] / 20) * 100, 100) ?>%"></div>
                                </div>
                            </div>
                            <span class="low-stock-count"><?= $p['stok'] ?>
                                <?= htmlspecialchars($p['satuan'] ?? 'pcs') ?></span>
                        </div>
                    <?php endforeach; ?>
                    ...
                    <div class="low-stock-item">
                        <div style="padding:24px;text-align:center;color:#9ca3af;font-size:12.5px;width:100%;">
                            <i class="fas fa-check-circle"
                                style="font-size:24px;color:#16a34a;display:block;margin-bottom:6px;"></i>
            ...
            <div class="dash-card">
                ...
                <div style="padding:16px; display:grid; grid-template-columns:repeat(2,1fr); gap:10px;">
                    <a href="<?= site_url('admin/penjualan/create') ?>"
                        ...
                        <span style="font-size:11.5px;font-weight:600;color:#1a56db;text-align:center;">Transaksi
                            Baru</span>
                    </a>
                    <a href="<?= site_url('admin/barang_masuk') ?>"
                        ...
                        <span style="font-size:11.5px;font-weight:600;color:#16a34a;text-align:center;">Riwayat
                            Masuk</span>
                    </a>
                    <a href="<?= site_url('admin/produk') ?>"
                        ...
                        <span style="font-size:11.5px;font-weight:600;color:#d97706;text-align:center;">Kelola
                            Stok</span>
                    </a>
