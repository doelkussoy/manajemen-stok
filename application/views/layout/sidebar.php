...
<style>
    .main-sidebar, .main-header, .sb-label, .sb-section-label, .brand-name, .brand-sub, .user-name, .user-greeting, .nav-link { font-family: 'DM Sans', 'Inter', sans-serif !important; }
    .main-sidebar { position: fixed !important; top: 0; bottom: 0; left: 0; width: var(--sidebar-width,260px); height: 100vh !important; overflow-y: auto; z-index: 1038; transition: width var(--transition-slow), transform var(--transition-slow), margin-left var(--transition-slow) !important; background: var(--bg-surface) !important; border-right: 1px solid var(--border-light); box-shadow: var(--shadow-sm); }
    .main-sidebar::-webkit-scrollbar { width: 4px; }
    .main-sidebar::-webkit-scrollbar-thumb { background: var(--border-medium); border-radius: 4px; }
    .sidebar-brand-wrapper { display: flex; align-items: center; justify-content: center; padding: 15px; border-bottom: 1px solid var(--border-light) !important; text-decoration: none !important; background: var(--bg-surface) !important; min-height: 100px; transition: all var(--transition-slow) !important; }
    .sidebar-logo-icon { height: 70px; display: flex; align-items: center; justify-content: center; transition: all var(--transition-slow) !important; }
    .sidebar-logo-icon img { height: 100%; width: auto; max-width: 180px; object-fit: contain; transition: all var(--transition-slow) !important; }
    .sidebar { background: var(--bg-surface) !important; display: flex !important; flex-direction: column; height: calc(100vh - 100px) !important; }
    nav.mt-1 { display: flex; flex-direction: column; flex: 1; padding-bottom: 10px; }
    .sb-section-label { display: block; padding: 14px 18px 5px; font-size: 9.5px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; color: var(--text-muted); }
    .sb-nav-list { list-style: none; padding: 0 8px !important; margin: 0 !important; }
    .sb-nav-list .nav-item { margin-bottom: 1px; }
    .main-sidebar .sb-nav-list .nav-link { display: flex !important; align-items: center !important; gap: 12px !important; padding: 10px 12px !important; margin-bottom: 4px !important; border-radius: var(--radius-md) !important; color: var(--text-body) !important; font-size: 13.5px !important; font-weight: 500 !important; text-decoration: none !important; transition: background-color var(--transition-normal), color var(--transition-normal), padding-left var(--transition-slow), box-shadow var(--transition-normal) !important; background: transparent !important; width: 100%; }
    .main-sidebar .sb-nav-list .nav-link:hover { background: var(--bg-surface-hover) !important; color: var(--color-primary) !important; padding-left: 16px !important; }
    .main-sidebar .sb-nav-list .nav-link.active { background: var(--bg-surface-hover) !important; color: var(--color-primary) !important; font-weight: 600 !important; box-shadow: var(--shadow-sm) !important; padding-left: 12px !important; border-left: 3px solid var(--color-primary); }
    .sb-icon { width: 30px; height: 30px; min-width: 30px; background: var(--bg-body); border-radius: var(--radius-md); display: flex !important; align-items: center; justify-content: center; font-size: 13px; color: var(--text-placeholder); transition: transform var(--transition-normal), background-color var(--transition-normal), box-shadow var(--transition-normal); }
    .main-sidebar .sb-nav-list .nav-link:hover .sb-icon { background: var(--color-primary-light); color: var(--color-primary); transform: scale(1.08); }
    .main-sidebar .sb-nav-list .nav-link.active .sb-icon { background: linear-gradient(135deg, var(--color-primary), var(--color-primary-hover)); color: #ffffff; box-shadow: var(--shadow-primary); }
    .sb-arrow { margin-left: auto; font-size: 10px; color: var(--border-medium); transition: transform var(--transition-slow); flex-shrink: 0; }
    .menu-open > .nav-link .sb-arrow, .nav-item.has-treeview.menu-is-opening > .nav-link .sb-arrow { transform: rotate(90deg); color: var(--color-primary); }
    .sb-label { flex: 1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display: block; }
    .sb-submenu { list-style: none !important; margin: 4px 0 8px 0 !important; padding: 0 0 0 16px !important; }
    .main-sidebar .sb-submenu .nav-link { gap: 10px !important; padding: 8px 12px !important; font-size: 13px !important; color: var(--text-muted) !important; border: none !important; margin-bottom: 2px !important; transition: color var(--transition-normal), background-color var(--transition-normal) !important; }
    .main-sidebar .sb-submenu .nav-link:hover { color: var(--color-primary) !important; background: var(--bg-surface-hover) !important; }
    .sb-dot { width: 6px; height: 6px; min-width: 6px; background: var(--border-medium); border-radius: 50%; transition: transform var(--transition-normal), background-color var(--transition-normal), box-shadow var(--transition-normal); }
    .main-sidebar .sb-submenu .nav-link:hover .sb-dot { background: var(--color-primary); }
    .main-sidebar .sb-submenu .nav-link.active { color: var(--color-primary) !important; font-weight: 600 !important; background: transparent !important; border: none !important; }
    .main-sidebar .sb-submenu .nav-link.active .sb-dot { background: var(--color-primary); transform: scale(1.4); box-shadow: 0 0 8px var(--color-primary-light); }
    .sb-divider { border: none; border-top: 1px dashed var(--border-light); margin-top: auto !important; margin-bottom: 14px; margin-left: 14px; margin-right: 14px; }
    .main-sidebar .sb-logout-link { display: flex !important; align-items: center !important; gap: 12px !important; padding: 10px 12px !important; margin: 0 8px 16px !important; border-radius: var(--radius-md) !important; color: var(--color-danger) !important; font-size: 13.5px !important; font-weight: 600 !important; text-decoration: none !important; transition: background-color var(--transition-normal), color var(--transition-normal), padding-left var(--transition-slow), box-shadow var(--transition-normal) !important; background: transparent !important; width: calc(100% - 16px); }
    .main-sidebar .sb-logout-link:hover { background: var(--color-danger-light) !important; padding-left: 16px !important; }
    .sb-icon-red { width: 30px; height: 30px; min-width: 30px; background: var(--color-danger-light); border-radius: var(--radius-md); display: flex; align-items: center; justify-content: center; font-size: 13px; color: var(--color-danger); transition: transform var(--transition-normal), background-color var(--transition-normal); }
    .main-sidebar .sb-logout-link:hover .sb-icon-red { transform: scale(1.08); background: var(--color-danger-border); }
